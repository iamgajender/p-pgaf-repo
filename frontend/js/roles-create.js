...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("create-role-view").hidden = false;
...
async function createRole() {
    ...
    setButtonBusy("create-role-btn", true, `<i class="fa-solid fa-user-shield"></i> Create Role`);
    ...
    try {
        ...
        if (!response.ok || result.status !== "success") {
            ...
            statusEl.textContent = result.message || "Could not create role.";
            return;
        }

        statusEl.className = "connection-status log-success";
        statusEl.textContent = result.message || `Role "${escapeHtml(payload.role_name)}" created successfully.`;
        ...
        setButtonBusy("create-role-btn", false, `<i class="fa-solid fa-user-shield"></i> Create Role`);
    }
}
